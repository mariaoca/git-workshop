...
describe("commit-branching", () => {
  it("should have commited a README.md file", () => {
    let files = execSync("git ls-tree --name-only master", {
      encoding: "utf8"
    });
    files = files.split("\n");
  ...
  it("should had merged feat/more-tests into master", () => {
    ...
    try {
      hasCommit = !!execSync(
        "git rev-parse --quiet --verify e0cc90e3197d5f118bc05dd5f7440a046d27f78f",
        { encoding: "utf8" }
      );
    } finally {
      assert(hasCommit);
  ...
  it("should have more commits", () => {
    let numberCommits = execSync("git rev-list --count master", {
      encoding: "utf8"
    });
